...
    var cur_reply_cnt = 0;
    ...
    // --- CODE MỚI ---
    // Biến tạm để lưu nước đi đầu tiên từ URL
...
        update_ws_status('Connected', '#00c853');
        ...
        // --- THÊM VÀO ĐÂY ---
        // Tự động bắt đầu game nếu có tham số trên URL
...
        render_board(null, null, true);
        ...
        // --- THÊM VÀO ĐÂY ---
        // Gửi nước đi đầu tiên (nếu có) sau khi game đã được server_start
        if (pending_first_move !== null) {
            ...
            if (coords && coords.r >= 0 && coords.r < board_size && coords.c >= 0 && coords.c < board_size) {
                console.log("Sending pending first move:", coords);
                socket_send("HM " + coords.r + " " + coords.c);
            }
            pending_first_move = null; // Xóa đi để không gửi lại
        }
        // --- KẾT THÚC THÊM VÀO ĐÂY ---
    }
...
    }
    ...
    // --- CODE MỚI ---
    // Hàm này xử lý việc đọc URL và tự động bắt đầu game
    function startGameFromURLParams() {
        const params = new URLSearchParams(window.location.search);
        const firstPlayer = params.get('first');
        const level = params.get('level');
        const firstMove = params.get('move');

        // Chỉ chạy nếu có tham số 'first' hoặc 'level'
        ...
        // 2. Cài đặt độ khó
        if (level) {
            let levelID = 'lv3'; // Mặc định 'Normal'
            switch (level.toLowerCase()) {
                ...
                case 'fuck me': levelID = 'lv6'; break;
            }
            $(`input[name='ai-level'][id='${levelID}']`).prop('checked', true);
        }

        // 3. Lưu nước đi đầu tiên (nếu có)
        if (firstPlayer === 'human' && firstMove) {
            const cellNum = parseInt(firstMove, 10);
            if (!isNaN(cellNum)) {
                pending_first_move = cellNum;
                console.log("Pending first move set to:", cellNum);
            }
        }

        // 4. Tự động bắt đầu game
        // Chúng ta gọi thẳng hàm `start_game()` thay vì click nút
        // vì hàm `on_loaded_wasm` đã đảm bảo `socket_send` sẵn sàng.
        start_game();
    }
    // --- KẾT THÚC CODE MỚI ---
...
    }
    ...
    // --- CODE MỚI ---
    // Hàm ngược lại: Lấy (r, c) từ số thứ tự ô
